...
class Im2ColConvFunctor {
 public:
  void operator()(OpKernelContext* context, const T1* input_data,
    ...
    const int64 max_chunk_size = (16 * 1024 * 1024);
    ...
    for (int64 chunk_index = 0; chunk_index < chunk_count; ++chunk_index) {
      ...
      ParallelFor(
          context, patch_index_start, patch_index_end,
          [output_height, output_width, input_data, input_height, input_width,
           input_depth, stride_rows, stride_cols, filter_top_offset,
           filter_left_offset, patches_per_chunk, im2col_buffer,
           filter_value_count, filter_height, filter_width](
              int64 task_patch_index_start, int64 task_patch_index_end) {
            for (int64 patch_index = task_patch_index_start;
                 patch_index < task_patch_index_end; ++patch_index) {
              ...
              const int in_x_origin =
                  (out_x * stride_cols) - filter_left_offset;
              const int patch_index_within_chunk =
                  patch_index % patches_per_chunk;
              T1* im2col_patch_start =
                  im2col_buffer +
                  (patch_index_within_chunk * filter_value_count);
              for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
                const int in_y = in_y_origin + filter_y;
                T1* im2col_row_start = im2col_patch_start +
                ...
                if ((in_y < 0) || (in_y >= input_height)) {
                  ...
                  // buffer as efficiently as possible, using functions to set
                  // or
                  // duplicate values en masse. We know we don't have to worry
                  // about
                  ...
                  // We use std::fill() to set the left and right sections to
                  // zeroes
                  ...
                  const int right_zero_count =
                      std::max(0, in_x_end - input_width);
                  ...
                  if (center_copy_count > 0) {
                    ...
                    std::copy(input_row_start, input_row_end,
                              im2col_center_start);
                  }
