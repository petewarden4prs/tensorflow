 ...
 public:
  void operator()(OpKernelContext* context, const Tensor& input,
    ...
    const int cache_end_x =
        ...
        filter_width;
    const int cache_line_width = cache_end_x - cache_start_x;
    const int cache_height =
        kResizeCacheSize / (cache_line_width * input_depth);
      ...
      for (int out_y = 0; out_y < output_height; ++out_y) {
        ...
        for (int out_x = 0; out_x < output_width; ++out_x) {
          ...
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            int cache_index_y;
            if (conv_in_y < 0) {
              cache_index_y = cache_height + (conv_in_y % cache_height);
            } else {
              cache_index_y = conv_in_y % cache_height;
            }
            T1* cache_line_start =
